...
- **欢迎页**：全屏进入页，必应每日一图背景 + 深色玻璃拟态卡片，打字机介绍、实时时钟、一言格言；标题/副标题/描述/按钮文字可编辑
- **个人简介**：左侧头像、名字、简介、链接，可编辑
- **标签导航**：内容按标签展示，标签可增删
- **文章 / 作品 / 分析**：Markdown 写作，支持代码块、图片、引用
- **站内搜索**：右上角搜索框，纯前端全文检索（Fuse.js）
- **AI 助手**：右下角桌宠式助手，可自定义头像（透明底图原样大图显示）与人格；回答从头顶冒气泡，下方输入框对话；本地检索站内内容 + Cloudflare Worker 代理 DeepSeek，支持联网搜索（可选，未配置则降级为站内问答）
...
在浏览器里编辑欢迎页、简介、标签、内容，点「保存」后，改动写入 `content/` 目录。点右上角「发布到线上」即可自动提交并推送（无需手动敲 git 命令）。
